...
import com.zrp.weixinpulltofresh.R;
...
//        mDecelerateInterpolator = new DecelerateInterpolator(DECELERATE_INTERPOLATION_FACTOR);
        mDecelerateInterpolator = new DecelerateInterpolator(0);
    ...
    private void createProgressView() {
        mCircleView = new CircleImageView(getContext(), CIRCLE_LIGHT);
//        mProgress = new MaterialProgressDrawable(getContext(), this);
        mProgress = new CustomProgressDrawable(getContext(), this);
        Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.weixinrotation);
        mProgress.setBitmap(bitmap);
        mProgress.setBackgroundColor(CIRCLE_LIGHT);
        mCircleView.setImageDrawable(mProgress);
...
//        mCircleView.setVisibility(View.GONE);
    ...
    void setRtotationAnimationProgress(float progress) {
        mCircleView.setRotation(progress);
    ...
    private void setRefreshing(boolean refreshing, final boolean notify) {
        if (mRefreshing != refreshing) {
            ...
            if (mRefreshing) {
                ...
                LogUtil.d("mRefreshing : " + refreshing);
                startUpAnimation(mRefreshListener);
                mProgress.stop();
            }
        }
    }
    ...
    private void moveTargetToStart() {
        mTarget.animate().translationY(0).setDuration(300).start();
    }
...
//                (width / 2 + circleWidth / 2), mCurrentTargetOffsetTop + circleHeight);
        // left = mSpinnerOffsetEnd - actionBarHeight;
        mCircleView.layout(childLeft + mSpinnerOffsetEnd - actionBarHeight, mCurrentTargetOffsetTop,
                childLeft + mSpinnerOffsetEnd + circleWidth - actionBarHeight, mCurrentTargetOffsetTop + circleHeight);
    }
    ...
    private void moveTarget(float mTotalUnconsumed) {
        mTarget.setTranslationY(mTotalUnconsumed);
    }
...
//        setTargetOffsetTopAndBottom(targetY - mCurrentTargetOffsetTop, true /* requires update */);
        //Target View 随着下拉上下移动
        mTarget.setTranslationY(overscrollTop);
        LogUtil.d("mTarget top : " + mTarget.getTop());
...
//        否则使用定义的值
        ...
        LogUtil.d("targetY : " + targetY + "endTarget" + endTarget);
        LogUtil.d("getTranslationY : " + mTarget.getTranslationY());

        if (Math.abs(targetY) >= endTarget) {
...
//                            startUpAnimation(null);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        LogUtil.d("onTouchEvent : " + ev.getAction());
        final int action = MotionEventCompat.getActionMasked(ev);
        int pointerIndex = -1;
...
//        mAnimateToCorrectPosition.setDuration(ANIMATE_TO_TRIGGER_DURATION);
        mAnimateToCorrectPosition.setDuration(2000);
